...
class SaleOrderLine(models.Model):
    ...
    def _timesheet_service_generation(self):
        ...
        so_line_new_project = self.filtered(lambda sol: sol.is_service and sol.product_id.service_tracking in ['project_only', 'task_in_project', 'gio_fieldservice_task'])
        ...
        if so_line_new_project:
            ...
            so_lines_with_project = self.search([('order_id', 'in', order_ids), ('project_id', '!=', False), ('product_id.service_tracking', 'in', ['project_only', 'task_in_project', 'gio_fieldservice_task']), ('product_id.project_template_id', '=', False)])
            map_so_project = {sol.order_id.id: sol.project_id for sol in so_lines_with_project}
            so_lines_with_project_templates = self.search([('order_id', 'in', order_ids), ('project_id', '!=', False), ('product_id.service_tracking', 'in', ['project_only', 'task_in_project', 'gio_fieldservice_task']), ('product_id.project_template_id', '!=', False)])
        ...
        def _determine_project(so_line):
            ...
            elif so_line.product_id.service_tracking == 'gio_fieldservice_task':
                return so_line.order_id.project_id or so_line.project_id
        ...
        for so_line in so_line_new_project:
            ...
            if not project and _can_create_project(so_line):
                if so_line.product_id.service_tracking == 'gio_fieldservice_task':
                    project = so_line._timesheet_create_fieldservice_project()
            ...
            #add the code for gio_fieldservice_task in project
            if so_line.product_id.service_tracking == 'gio_fieldservice_task':
                if not project:
                    if so_line.product_id.project_template_id:
                        project = map_so_project_templates[(so_line.order_id.id, so_line.product_id.project_template_id)]
                    else:
                        project = map_so_project[so_line.order_id.id]
                if not so_line.task_id:
                    so_line._timesheet_create_task(project=project)
    ...
    def _timesheet_create_fieldservice_project(self):
